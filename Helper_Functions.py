...
def im_and_segm_mesh_hernan(crop_x_in, crop_x_end, crop_y_in, crop_y_end, data_path):
    ...
    vol_mesh = meshio.read( os.path.join(data_path, volunteer+'_vol.vtu') )
    # Segmentation mesh
...
def generate_background_mesh(data_path, imt_data, crop_x_in, crop_x_end, crop_y_in, crop_y_end, slices):
    ...
    vol_file = os.path.join( data_path, volunteer+'_vol.vtu')
    vol_mesh = meshio.read(vol_file)
    n_points = len(vol_mesh.points) 

    #bg_file = os.path.join( data_path, volunteer+'_bg.vtk')
    bg_file = os.path.join( data_path, volunteer+'_surf_vol.vtu')
    bg_mesh = meshio.read(bg_file)
    cells = bg_mesh.cells   #changed from .cells[1][1]
    ...
    for point_mesh in tqdm(candidate_mesh):
...
def generate_boolean_mask(data_path, imt_data, crop_x_in, crop_x_end, crop_y_in, crop_y_end):
    ...
    bg_file = os.path.join( data_path, volunteer+'_surf_vol.vtu')
    bg_mesh = meshio.read(bg_file)
    cells = bg_mesh.cells   #changed from [1][1] to just cells
    nodes = bg_mesh.points
    ...
    for i in tqdm(range(len(im_mesh))):
...
def compare_images(ground_truth, prediction):
    ...
    ssim_im = ssim(ground_truth, prediction, data_range=ground_truth.max() - ground_truth.min(), multichannel=True, win_size=3)  #Added data range and multichannel parameter to True and win_size = 3
...
def mse_ssim(model_dir, frames, mse_pred_list, mse_temp_list, mse_wartemp_list, ssim_pred_list, ssim_temp_list, ssim_wartemp_list):
    ...
    ax[0].plot(np.arange(1, frames+1), mse_wartemp_list, label='Template vs Warped Template')
    ...
    ax[1].plot(np.arange(1, frames+1), ssim_wartemp_list, label='Template vs Warped Template')
    ...
    ax[2].set_title('DICE')
    #ax[2].plot(np.arange(1, frames+1), dice_pred_list, label='Reference vs Warped Template')
    ax[2].plot(np.arange(1, frames+1), dice_wartemp_list, label='Template vs Warped Template')
    ax[2].set_xticks(np.arange(1, frames+1, 2))
...
def global_strain(data_path, model_dir, frames, pinn_strains):
    ...
    """ # commented out inria and upf portion
    ...
    for i in range(0, frames):
        ...
        E_rr = strain_inria.point_data['radStrain']             #edited from point_arrays to point_data; point_arrays(deprecated)
        E_cc = strain_inria.point_data['circStrain']            #edited from point_arrays to point_data; point_arrays(deprecated)
        E_ll = strain_inria.point_data['longStrain']            #edited from point_arrays to point_data; point_arrays(deprecated)
        ...
        E_rr = strain_upf.point_data['radStrain']               #edited from point_arrays to point_data; point_arrays(deprecated)
        E_cc = strain_upf.point_data['circStrain']              #edited from point_arrays to point_data; point_arrays(deprecated)
        E_ll = strain_upf.point_data['longStrain']              #edited from point_arrays to point_data; point_arrays(deprecated)
...
def plot_image(X, Y, Z, path_fig, image_pred, title, name, levels, vmin, vmax, slices, save=True):
    fig, axes = plt.subplots(len(slices), figsize=(12,5))
