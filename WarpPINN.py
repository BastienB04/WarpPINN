###################################################################################################
## NOTE: This code is adopted and based upon the original, publicly available code by López et al.
...
class WarpPINN:    
    def __init__(self, imr, imt, layers_u, bool_mask, im_mesh, segm_mesh, bg_mesh, lmbmu, pix_crop, reg_mask=1, sigma=None
                 ...
                 , physics_loss_OFF=False
                 , physics_mask_OFF=False):
        ...
        #  *Saving architecture as a check for loading imported weights and biases* #
        ...
        ### =====================================================================================================
        ### SimpleITK Initial Alignment ###
        self.simpleitk_transform = simpleitk_transform
        if self.simpleitk_transform:
            # Perform initial SimpleITK registration
            self.initial_transforms, self.imt_transformed = self.simpleitk_register_to_multiple(self.imr, self.imt)

        ### =====================================================================================================

        ### =====================================================================================================
        ### Setting Physics weight off ###
        self.physics_loss_OFF = physics_loss_OFF
        self.physics_mask_OFF = physics_mask_OFF
        ...
        ### =====================================================================================================
        ### Setting Physics weight off ###
        if self.physics_mask_OFF:
            self.loss_MSE = tf.compat.v1.reduce_mean( tf.square( (self.imr_pred - self.imr_tf) ) )
            self.loss_L1 = tf.compat.v1.reduce_mean( tf.abs( (self.imr_pred - self.imr_tf) ) )

            self.loss_MSE_crop = tf.compat.v1.reduce_mean( tf.square(self.imr_crop_pred - self.imr_crop_tf))
            self.loss_L1_crop = tf.compat.v1.reduce_mean( tf.abs(self.imr_crop_pred - self.imr_crop_tf))
        else:
    ...
    def simpleitk_register(self, imr, imt):
        # Convert numpy arrays to SimpleITK images
        imr_sitk = sitk.GetImageFromArray(imr)
        imt_sitk = sitk.GetImageFromArray(imt)

        # Initialize the registration method
        registration_method = sitk.ImageRegistrationMethod()

        # Set the registration components: optimizer, metric, etc.
        ...
        # Use an initial transformation (e.g., centering)
        initial_transform = sitk.CenteredTransformInitializer(imr_sitk, imt_sitk, sitk.Euler3DTransform(), sitk.CenteredTransformInitializerFilter.GEOMETRY)
        registration_method.SetInitialTransform(initial_transform)

        # Execute the registration
        final_transform = registration_method.Execute(imr_sitk, imt_sitk)

        # Apply the transformation to 'imt' for visualization or further use
        resampler = sitk.ResampleImageFilter()
        resampler.SetReferenceImage(imr_sitk)
        resampler.SetTransform(final_transform)
        imt_transformed_sitk = resampler.Execute(imt_sitk)
        imt_transformed = sitk.GetArrayFromImage(imt_transformed_sitk)

        return final_transform, imt_transformed

    def simpleitk_register_to_multiple(self, imr, imt):
        final_transforms_array = []
        imt_transformed_array = []

        for i in range(imt.shape[0]):  # Looping through each time frame
            imt_frame = imt[i, :, :, :]

            # Perform registration (reuse the simpleitk_register logic here)
            final_transform, imt_transformed = self.simpleitk_register(imr, imt_frame)

            final_transforms_array.append(final_transform)
            imt_transformed_array.append(imt_transformed)

        return final_transforms_array, np.array(imt_transformed_array)
    ### =====================================================================================================


    '''
    def simpleitk_register(self, fixed_image, moving_image):
        # Configure the registration method
        registration_method = sitk.ImageRegistrationMethod()

        # Set initial alignment such as centering
        initial_transform = sitk.CenteredTransformInitializer(fixed_image
                                                              , moving_image
                                                              , sitk.Euler3DTransform()
                                                              , sitk.CenteredTransformInitializerFilter.GEOMETRY )

        ## Set registration parameters: metric, optimizer, etc.

        # Metric: Choose Mean Squares for intensity-based similarity
        registration_method.SetMetricAsMeanSquares()

        # Optimizer: Use Regular Step Gradient Descent
        registration_method.SetOptimizerAsRegularStepGradientDescent(learningRate=2.0, minStep=0.001, numberOfIterations=200)

        # Interpolator: Use Linear Interpolator
        registration_method.SetInterpolator(sitk.sitkLinear)

        # Initializer: Center the images using moments
    ...
    def apply_simpleitk_transform(self, input_image):   # Do you need to call this in init
        if self.simpleitk_transform is not None:
            # Apply the transformation to the input image
            resampler = sitk.ResampleImageFilter()
            resampler.SetReferenceImage(input_image)
            resampler.SetTransform(self.simpleitk_transform)
            transformed_image = resampler.Execute(input_image)
    ...
    # L1 + NEO HOOKEAN
    def train_Adam_L1_NeoHook(self, nEpoch, mu_NeoHook
                              , save_csv_path
                              , size=100, crop=False
                              ): 
        """ 
        ...
        ### Implementing saving of training results to CSV =========================================
        with open(save_csv_path, mode='w', newline='') as file:
            writer = csv.writer(file)
            # Writing the header
            writer.writerow(["Epoch", "Iteration", "Loss", "Loss L1", "Loss NeoHook", "Time"])
        ### ========================================================================================
        ...
        for ep in range(nEpoch):
            ...
            for it, idx in enumerate(splits):
                ...
                with open(save_csv_path, mode='a', newline='') as file:
                    writer = csv.writer(file)
                    # Writing the header
                    writer.writerow([ep, iter, loss_value, loss_value_L1, loss_value_NH, elapsed])
                ### ========================================================================================
...
############ UNUSED METHODS #######################
    ...
    ### =====================================================================================================
    ### TRANSFER LEARNING ###
    def save_WeightsAndBiases(self, save_path):
        '''
        - Save the current WarpPINN model's weights and biases as a JSON file, such that it can be loaded by 
        the equivalent load function.
        - In order to seralise the weights_u and biases_u data types, they must be converted into a numpy array.
        - Conversion to numpy array requires evaluation variables within a tensorflow enironment where Eager Execution
        is not disabled (it is for the WarpPINN code).
        - We also save layers_u as a means of verifying that the architectures first match before loading weights and 
        biases from an imported source in the equivalent load function.
        '''

        layers_list = np.array(self.layers_u).tolist()

        with tf.compat.v1.Session() as sess:
            # Initialize variables if they haven't been initialized yet
            sess.run(tf.compat.v1.global_variables_initializer())
            
            # Evaluate the variables and convert them to numpy arrays
            weights = [sess.run(w) for w in self.weights_u]
            biases = [sess.run(b) for b in self.biases_u]

        weights_list = [w.tolist() for w in weights]
        biases_list = [b.tolist() for b in biases]

        # Save to JSON
        with open(save_path, 'w') as f:
            json.dump({'layers': layers_list
                       ,'weights': weights_list
                       , 'biases': biases_list}
                       , f)

    def load_WeightsAndBiases(self, load_path):
        # Load from JSON
        ...
        biases_np = [np.array(b) for b in biases_list]

        if not np.array_equal(np.array(self.layers_u), np.array(layers_list)):
            raise ValueError('Architectures of the current WarpPINN model and the model whose weights and biases are being loaded do not match.')
        else:
            # Assuming new_model is your new TensorFlow model with the same architecture
            for i, w in enumerate(self.weights_u):
                w.assign(weights_np[i])

            for i, b in enumerate(self.biases_u):
                b.assign(biases_np[i])
